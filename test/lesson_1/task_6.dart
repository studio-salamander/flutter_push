...
void runTestLesson1Task6() {
  testWidgets('module6', (WidgetTester tester) async {
    ...
    final List<Stack> stackWidgets = tester
        .widgetList<Stack>(find.byWidgetPredicate((widget) => widget is Stack))
        .toList();
    ...
    final Widget? topWidget = stackWithThreeWidgets!.children.firstWhere(
        (element) =>
            element is Align && element.alignment == const Alignment(0, -0.8));
    expect(topWidget, isNotNull);
    final Widget? bottomWidget = stackWithThreeWidgets.children.firstWhere(
        (element) =>
            element is Align && element.alignment == const Alignment(0, 0.8));
